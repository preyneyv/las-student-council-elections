...
export class AppComponent implements OnInit {
  ...
  pinInput(evt, val, e) {
    if (val.length === 4) {
      ...
      // Check the pin
      this.http.get<any>(url(`teachers/${val}`)).subscribe(
        (res) => {
          // valid pin
          const { teacher } = res;
          if (!confirm(`Are you ${teacher.name}?`)) {
  ...
  loadPositions() {
    ...
    this.http.get<any>(url(`teachers/${this.teacher.pin}/positions`)).subscribe(
  ...
  submitVotes() {
    ...
    this.http.post<any>(url(`teachers/${this.teacher.pin}`), choices).subscribe(
      () => {
