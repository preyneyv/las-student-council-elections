...
export class AppComponent implements OnInit {
  ...
  pinInput(evt, val, e) {
    if (val.length === 4) {
      ...
      // Check the pin
      this.http.get<any>(url(`students/${val}`)).subscribe(
        (res) => {
          // valid pin
          const { student } = res;
          if (!confirm(`Are you ${student.name}?`)) {
  ...
  loadPositions() {
    ...
    this.http.get<any>(url(`students/${this.student.pin}/positions`)).subscribe(
  ...
  submitVotes() {
    ...
    this.http.post<any>(url(`students/${this.student.pin}`), choices).subscribe(
      () => {
